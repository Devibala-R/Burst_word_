...
public class ChildEnter1 : MonoBehaviour
{
   ...
   /* IEnumerator ShowWithDelay()
        ...
        if (childInput.text.Length > 0)
        {
            ...
            if (childInput.text[letterCheckIndex] == go.name[0])
            {
                ...
                if (childInput.text.Length == TeacherEnter.wordLength[typedWordIndex])
                {
                    ...
                    childInput.text = "";
                    letterCheckIndex = 0;
                    Debug.Log("typedwordIndex" + typedWordIndex);
                    typedWordIndex++;
                    Debug.Log("typedwordIndex" + typedWordIndex);
    ...
    IEnumerator ShowWithDelay()
    {
        ...
        {
             showWord.SetTrigger("show");
          /*  showWord.gameObject.transform.DOLocalMoveY(300, 6);
            yield return new WaitForSeconds(6f);

            showWord.gameObject.transform.DOLocalMoveY(601, 6);*/
            yield return new WaitForSeconds(2f);

            playerFly.SetTrigger("playerFly");
            yield return new WaitForSeconds(3f);
            BottomCollider.b = false;
            spawnPositions.transform.GetChild(typedWordIndex).gameObject.SetActive(true);
    ...
    public void CollisionDeleteWord()
    {
        ...
        Debug.Log("typedwordindex before " + typedWordIndex);

        if (!spawnPositions.transform.GetChild(typedWordIndex).gameObject.activeSelf)
        {
           typedWordIndex++;
          lossScore();
        }
        Debug.Log("typedwordindex after " + typedWordIndex);
        childInput.text = "";
        //  letterCheckIndex = 0;
